...
static
t_std_error ndi_create_bridge_port(npu_id_t npu_id, ndi_brport_obj_t *info, sai_object_id_t bridge_oid, sai_object_id_t *br_port_id) {
        ...
        case ndi_brport_type_SUBPORT:
            {
                bridge_port_attr[attr_idx].id = SAI_BRIDGE_PORT_ATTR_PORT_ID;
                bridge_port_attr[attr_idx++].value.oid = info->port_obj_id;
                sai_uint16_t sai_vlan_id= info->vlan_id;
...
static t_std_error ndi_delete_all_type_subports(npu_id_t npu_id, sai_object_id_t brport_oid, hal_vlan_id_t  vlan_id)
{
   ...
   blk1.brport_type = ndi_brport_type_SUBPORT;
...
ndi_1d_bridge_member_lag_add(npu_id_t npu_id, sai_object_id_t br_oid, ndi_obj_id_t ndi_lag_id, hal_vlan_id_t vlan_id, bool tag)
{
    ...
    info.port_obj_id = ndi_lag_id;
    info.port_type = ndi_port_type_LAG;
    info.brport_type = ndi_brport_type_SUBPORT;
    info.vlan_id = vlan_id;

    if ((rc =  ndi_create_bridge_port(npu_id, &info, br_oid, &bridgeport_id)) != STD_ERR_OK) {
       NDI_PORT_LOG_ERROR("ADD 1D LAG bridgeport failed. br_oid % " PRIx64 " ",
        br_oid);
       return rc;

    }
    bridge_port_attr.id = SAI_BRIDGE_PORT_ATTR_TAGGING_MODE;
    if (tag) {
        bridge_port_attr.value.u32 = (sai_bridge_port_tagging_mode_t)SAI_BRIDGE_PORT_TAGGING_MODE_TAGGED;
    } else {
        bridge_port_attr.value.u32 = (sai_bridge_port_tagging_mode_t)SAI_BRIDGE_PORT_TAGGING_MODE_UNTAGGED;
    }
    NDI_PORT_LOG_TRACE("Add Lag to 1D br %" PRIx64 " " "lag_id  %" PRIx64 " " "is_tag %d, vlan_id %d",
                                                                    br_oid, ndi_lag_id, tag, vlan_id);
    if (ndi_set_bridge_port_attribute (npu_id, bridgeport_id, &bridge_port_attr) != STD_ERR_OK) {
        return STD_ERR(NPU, CFG, 0);
    }
    return STD_ERR_OK;
...
ndi_1d_bridge_tunnel_port_add(npu_id_t npu_id, sai_object_id_t br_oid, sai_object_id_t tunnel_oid, sai_object_id_t *bridge_port_id)
{
    ...
    memset(&info, 0, sizeof(ndi_brport_obj_t));
    info.port_obj_id = tunnel_oid;
    info.port_type = ndi_port_type_PORT;
    info.brport_type = ndi_brport_type_TUNNEL;
    return ndi_create_bridge_port( npu_id, &info, br_oid, bridge_port_id);
}
...
ndi_1d_bridge_member_port_add(npu_id_t npu_id, sai_object_id_t br_oid , npu_port_t port, hal_vlan_id_t vlan_id, bool tag) {
    ...
    info.port_obj_id = sai_port;
    info.port_type = ndi_port_type_PORT;
    info.brport_type = ndi_brport_type_SUBPORT;
    info.vlan_id = vlan_id;

    if((rc  = ndi_create_bridge_port( npu_id, &info, br_oid, &br_po_id)) != STD_ERR_OK) {
    ...
    if (ndi_set_bridge_port_attribute (npu_id, br_po_id, &bridge_port_attr) != STD_ERR_OK) {
        return STD_ERR(NPU, CFG, 0);
    }
    return STD_ERR_OK;
}
...
t_std_error nas_ndi_create_bridge_port_1Q(npu_id_t npu_id, sai_object_id_t sai_port, bool lag) {
     ...
     return ndi_create_bridge_port(npu_id, &blk, default_1q_br_oid, &br_po_id);
}
